...
class otherUserFollowers extends Component {
 constructor(props){
 super(props);
...
//Method to get list of followers when the logged
//views another user's profile
getListOfFollowers(){
  //this.props.navigation.getParam('userIdCallback') is a paramater that is sent from
  //the OtherUserProfile screen when the followers text field is clicked on
  return fetch('http://10.0.2.2:3333/api/v0.0.5/user/' + this.props.navigation.getParam('userIdCallback') + '/followers', {
    method: 'GET',
    headers: {
